...
categ <- distinct(products[, c(1,5,9)], id, Producto, Used)
...
colnames(categ) <- c("Product","Category","Used")
dbase <- merge(dbase, categ, by= "Product")  
head(dbase)
dbase$Category.y <- NULL
...
dbase$Varieties <- 0
dbase$Varieties <- with(dbase, ave(Product, Category, Time, Super, FUN= length)) - 1
...
# depto.number: the id of the state (departamento) where the store is located (not used)
